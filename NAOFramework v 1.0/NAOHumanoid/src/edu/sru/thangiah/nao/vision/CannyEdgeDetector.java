...
public class CannyEdgeDetector {
	...
	public void setSourceImage(BufferedImage image , boolean resize) {
		this.resize = resize;
		if(resize){
		sourceImage = scaleDown(image);	
		}
		else{
		sourceImage = image;
		}
	...
	private void thresholdEdges() {
		...
		for (int i = 0; i < picsize; i++) {
			...
			alpha[i] = alpha[i] > 0 ? -1: 0x00000000;
		}
	}
	...
	private void writeAlpha(int pixels[]){
		//NOTE: There is currently no mechanism for obtaining the edge data
		//in any other format other than an INT_ARGB type BufferedImage.
		//This may be easily remedied by providing alternative accessors.
		if(alphaImage == null || alphaImage.getWidth() != width){
		alphaImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
		}
		alphaImage.getWritableTile(0, 0).setDataElements(0, 0, width, height, pixels);
	}
	...
	private void writeEdges(int pixels[]) {
		...
		//in any other format other than an INT_ARGB type BufferedImage.
		//This may be easily remedied by providing alternative accessors.
		if(edgesImage == null || edgesImage.getWidth() != width){
